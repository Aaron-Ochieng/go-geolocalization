...
func TestGetRelations(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		// Check if the correct endpoint is being called
		if r.URL.String() != "/api/relation" {
			t.Errorf("Expected to request '/api/relation', got: %s", r.URL.String())
		}

		// Create mock relations data based on the provided sample
		mockRelations := utils.RelationsIndex{
			Index: []utils.Relations{
				{
					ID: 1,
	...
	// Call the function we're testing
	result, err := utils.GetRelations(server.URL + "/api/relation")
	// Check for errors
	if err != nil {
	...
	// Compare the result with the expected output
	if !reflect.DeepEqual(result, expected) {
		t.Errorf("GetRelations() = %v, want %v", result, expected)
	}
}

func TestGetRelationsError(t *testing.T) {
	// Test with invalid URL
	_, err := utils.GetRelations("http://invalid-url")
	if err == nil {
		t.Error("Expected error with invalid URL, got nil")
	}
	...
	_, err = utils.GetRelations(server.URL)
	if err == nil {
		t.Error("Expected error with server error, got nil")
	}
	...
	_, err = utils.GetRelations(serverInvalidJSON.URL)
	if err == nil {
		t.Error("Expected error with invalid JSON, got nil")
	}
}
